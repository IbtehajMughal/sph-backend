...
// Doesn't need this as every Chromium instance works same like incognito mode or private mode so needs to LOG IN each time with each instance.
async function getAuthFacebook() {
  ...
  try {
    const browser = await puppeteer.launch({
      ...
      // userDataDir: 'C:/Users/ibteh/AppData/Local/Google/Chrome/User Data',
    });
...
async function loginFacebook(user, { email, pass }) {
  ...
  try {
    ...
    if (
      currentUrl === 'https://www.facebook.com/?sk=welcome' ||
      currentUrl === 'https://www.facebook.com/' ||
      currentUrl === 'https://web.facebook.com/?sk=welcome'
    ) {
...
async function scrapeMarketPlaceData(browserInstance, user) {
  try {
    ...
    let drop = await marketPage.waitForSelector(
    ...
    await marketPage.click(
      'div.xu96u03.xm80bdy.x10l6tqk.x13vifvy > div.x1jx94hy.x1lq5wgf.xgqcy7u.x30kzoy.x9jhf4c.xbsqzb3.x9f619.x78zum5.xdt5ytf.x1iyjqo2.xr9ek0c.xh8yej3 > div > ul > li'
    );
...
async function getFbListing(user) {
  ...
  let newFacebookData;
  newFacebookData = await loginFacebook(user, { email: 'ibbe1122@yahoo.com', pass: 'Uk@6473235' });
  // if (isLoggedIn === undefined || isLoggedIn === null) {
  //   newFacebookData = await loginFacebook(user, { email: 'hsmart1122@yahoo.com', pass: 'Uk@6473235' });
  // }

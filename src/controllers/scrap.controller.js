...
const startScrap = catchAsync(async (req, res) => {
  ...
  let userIntrests = user?.interests?.map((us) => {
    if (us.platform === 'Crage List' || us.platform === 'Facebook') return us;
  });
  let interest = userIntrests[0];
  console.log(interest);
  const currentScrapJob = await scrapJobService.createScrapJob({
    status: 'pending',
    platform: user.platform,
    searchedFor: interest?.keywords,
  });

  if (interest?.platform === 'Crage List') {
    cragListService
      .getCraigListing(interest)
